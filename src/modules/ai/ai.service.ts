...
class AIService {
  ...
  // 📌 **AI'ye mesaj gönder ve yanıt al**
  async askAI(text: string, userId: string): Promise<string> {
    try {
      const response = await openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [
          { role: "system", content: "Sen bir yardımcı AI'sin." },
  ...
  async processConversation(userId: string, deviceId: string, text: string): Promise<string> {
    try {
      ...
      await ConversationService.addMessage(userId, deviceId, "user", text); // Kullanıcının mesajı
